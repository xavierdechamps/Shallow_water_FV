...
!  write ( command, * ) 'gnuplot ' // trim ( command_filename ) // ' &'
...
!  write ( *, '(a)' ) '  Press RETURN to proceed.'
  ...
  if ( status .ne. 0 .or. .not.lreturn ) then
    write ( *, '(a)' ) ' '
...
!
  ...
  do i = 1, nrow
    write ( file_unit, '(F6.0,3(1X,E15.8))' ) x(i,1:ncol)
  end do
...
!  Write the data file.
!
  character(len=3), allocatable :: headers(:)
  allocate(headers(4))
  headers(1)="index"
  headers(2)="h"
  headers(3)="h*u"
  ...
  write ( file_unit, '(a)' ) 'set term qt title "Gnuplot"'
!  write ( file_unit, '(a)' ) 'set term wxt title "Gnuplot"'
!  write ( file_unit, '(a)' ) 'set title "GNUFOR plot"'
!  write ( file_unit, '(a)' ) 'set xlabel "x"'
!  write ( file_unit, '(a)' ) 'set ylabel "y"'
  write ( file_unit, '(a)' ) 'set grid'
  write ( file_unit, '(a)' ) 'set logscale y'
  write ( file_unit, '(a)' ) 'set format y "10^{%03T}"'
  do i = 2, ncol
    if (i.eq.2) then
        write ( file_unit, '(a,i2,a)' ) 'plot "' // trim ( data_filename ) // &
        '" u ', i, ' w l title "' // trim (headers(i)) // '"'
    else
        write ( file_unit, '(a,i2,a)' ) 'replot "' // trim ( data_filename ) // &
        '" u ', i, ' w l title "' // trim (headers(i)) // '"'
    endif
    !write ( file_unit, '(a)' ) 'pause -1'
  end do
